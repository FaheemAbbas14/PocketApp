...
fun EventCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Text("📅 ${formatDateTime(event.eventDateMillis)}", style = MaterialTheme.typography.labelMedium)
        }
    }
...
fun EditEventDialog(event: EventItem, viewModel: MainViewModel, onDismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(onClick = {
                val eventDateMillis = mergeDateAndTimeMillis(selectedDate, selectedHour, selectedMinute)
                viewModel.updateEvent(event.copy(title = title, description = description, eventDateMillis = eventDateMillis, alarmEnabled = alarmEnabled))
                onDismiss()
            }) { Text("Update") }
        },
        dismissButton = { TextButton(onClick = onDismiss) { Text("Cancel") } }
